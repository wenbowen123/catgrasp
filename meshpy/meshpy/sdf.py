...
class Sdf():
    ...
    def is_out_of_bounds(self, coords):
        ...
        """
        return np.array(coords < 0).any() or np.array(coords >= self.dims_.reshape(3,1)).any()
...
class Sdf3D(Sdf):
    ...
    def __init__(self, sdf_data, origin, resolution, use_abs=False,
        ...
        self.data_ = sdf_data  #(dim,dim,dim)
        self.data_torch = torch.from_numpy(self.data_).float()
        self.origin_ = origin
        self.resolution_ = resolution
        self.dims_ = np.array(self.data_.shape)
        ...
        center_sdf = self.data_[self.dims_[0]//2,self.dims_[1]//2,self.dims_[2]//2]
        print("Sdf3D self.dims_={}, self.resolution_={}, self.origin_={}, center_sdf={}, boundary_sdf={}".format(self.dims_,self.resolution_,self.origin_, center_sdf, self.data_[0,0,0]))
    ...
    def _signed_distance(self, coords, fast=False):
        """Returns the signed distance at the given coordinates, interpolating
        if necessary.

        Parameters
        ----------
        coords : :obj:`numpy.ndarray` of int (3,N)
        ...
        for i in range(3):
            coords[i] = np.clip(coords[i],0,self.dims_[i]-1)

        if fast:
            coords = coords.round().astype(int)
            sd = self.data_[coords[0],coords[1],coords[2]]
            return sd

        min_coords = np.floor(coords)  #(3,N)
        max_coords = min_coords+1
        corners = np.zeros((coords.shape[1],Sdf3D.num_interpolants,3),dtype=float)  #(N,8,3)
        corners[:,Sdf3D.min_coords_x,0] = min_coords[0].reshape(-1,1)
        corners[:,Sdf3D.max_coords_x,0] = max_coords[0].reshape(-1,1)
        corners[:,Sdf3D.min_coords_y,1] = min_coords[1].reshape(-1,1)
        corners[:,Sdf3D.max_coords_y,1] = max_coords[1].reshape(-1,1)
        corners[:,Sdf3D.min_coords_z,2] = min_coords[2].reshape(-1,1)
        corners[:,Sdf3D.max_coords_z,2] = max_coords[2].reshape(-1,1)
        sd = np.zeros((coords.shape[1]),dtype=float)
        corners = corners.astype(int)
        for i in range(Sdf3D.num_interpolants):
            cur_corner = corners[:,i]  #(N,3)
            out_bounds_mask = (cur_corner<0).any(axis=1) | (cur_corner>=self.dims_.reshape(1,3)).any(axis=1)
            in_bound_mask = out_bounds_mask==0
            cur_values = np.zeros((len(cur_corner)))
            cur_values[in_bound_mask] = self.data_[cur_corner[in_bound_mask,0],cur_corner[in_bound_mask,1],cur_corner[in_bound_mask,2]]
            weights = np.prod(1-np.abs(cur_corner-coords.T), axis=1)  #(N)
            sd = sd+weights*cur_values

        return sd

    def _signed_distance_batch(self,coords,fast=True):
        '''
        @coords: (B,3,N) torch.tensor
        '''
        B = coords.shape[0]
        self.data_torch = self.data_torch.to(coords.device)  #(D,D,D)
        if fast:
            coords = torch.round(coords).long()
            coords[:,0] = torch.clamp(coords[:,0],0,self.dims_[0]-1)
            coords[:,1] = torch.clamp(coords[:,1],0,self.dims_[1]-1)
            coords[:,2] = torch.clamp(coords[:,2],0,self.dims_[2]-1)
            sd = self.data_torch[coords[:,0],coords[:,1],coords[:,2]]  #(B,N)
            return sd
        else:
            raise NotImplemented


    def transform_pt_obj_to_grid_batch(self, x_sdf_batch, direction=False):
        """ Converts a point in sdf coords to the grid basis. If direction then don't translate.

        @x_sdf_batch: (B,3,N) tensor
        """
        B,_,N = x_sdf_batch.shape
        T_world_grid = copy.deepcopy(self.T_world_grid_.matrix)
        T_world_grid_batch = torch.tensor(T_world_grid).float().to(x_sdf_batch.device).unsqueeze(0).repeat(B,1,1)
        ones = torch.ones((B,1,N)).float().to(x_sdf_batch.device)
        x_sdf_batch_homo = torch.cat((x_sdf_batch,ones), dim=1)
        x_grid = torch.bmm(T_world_grid_batch, x_sdf_batch_homo)[:,:3]
        return x_grid



    def is_any_points_inside(self,coords):
        '''
        @coords: (3,N)
        '''
        coords = coords.round().astype(int)
        coords = coords[:,coords[0]>=0]
        coords = coords[:,coords[0]<self.dims_[0]]
        coords = coords[:,coords[1]>=0]
        coords = coords[:,coords[1]<self.dims_[1]]
        coords = coords[:,coords[2]>=0]
        coords = coords[:,coords[2]<self.dims_[2]]
        sd = self.data_[coords[0],coords[1],coords[2]]
        return (sd<0).any()
    ...
    def find_zero_crossing_quadratic(x1, y1, x2, y2, x3, y3, eps=1.0):
        ...
        @x1,x2,x3: three 3d-points along the line
        """
        # compute coords along 1d line
        v = x2 - x1
        v = v / np.linalg.norm(v)
        if v[v != 0].shape[0] == 0:
            raise RuntimeError('Difference is 0. Probably a bug')
        ...
        try:
            ...
            raise RuntimeError('Singular matrix. Probably a bug')
