...
class Voxelization_Idx(Function):
    ...
    def forward(ctx, coords, batchsize, mode=4):
        ...
        :return: output_map: int M * (maxActive + 1), voxel to point map
...
class BallQueryBatchP(Function):
    ...
    def forward(ctx, coords, batch_idxs, batch_offsets, radius, meanActive):
        ...
        :return: idx (nActive), int, stores neighbor indices (among n) within radius of each n point, flattened
        :return: start_len (n, 2), int. First is start pos (in idx) of the point, second is number of neighbors
        '''
...
class BFSClusterKdtree(Function):
    @staticmethod
    def forward(ctx, xyz_tensor, semantic_label, batch_offsets, threshold, search_radius):
        '''
        :param ctx:
        :param semantic_label: (N), int
        :param start_len: (N, 2), int
        ...
        assert xyz_tensor.is_contiguous()
        ...
        PG_OP.bfs_cluster_kdtree(xyz_tensor, semantic_label, batch_offsets, cluster_idxs, cluster_offsets, N, threshold, search_radius)
...
bfs_cluster_kdtree = BFSClusterKdtree.apply
